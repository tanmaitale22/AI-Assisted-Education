...
def find_existing_node(node_name):

    safe_name = create_node_name(node_name)

    # Search across loaded ontologies
    return default_world.search_one(
        iri="*" + safe_name
    )
...
def build_daa_graph(extracted_knowledge):
    ...
    # Load existing graph into Owlready2's world
    load_existing_graph()
    ...
    def resolve_node(name):
        ...
        return find_existing_node(name)
    ...
    # --------------------------------
    # Save ALL knowledge from schema ontology
    # --------------------------------
